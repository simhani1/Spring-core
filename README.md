...
#### ObjectProvider의 활용
- HTTP 요청이 들어오고 나서 getObject 함수를 실행해서 request 스코프 빈을 생성하므로 문제가 발생하지 않는다.

#### 스코프와 프록시
- CGLIB라는 라이브러리로 내 클래스를 상속받은 가짜 프록시 객체를 만들어서 주입한다.
- 즉 MyLogger객체를 상속받은 가짜 프록시 객체를 생성한다.
- 가짜 프록시 객체는 요청이 오면 그때 내부에서 진짜 빈을 요청하는 위임 로직이 들어있다.
- 특징
  - 클라이언트는 마치 싱글톤 빈을 사용하듯이 편리하게 request scope를 사용할 수 있다.
  - provide, 프록시를 사용하는 것의 핵심은 진짜 객체를 조회를 곡 필요한 시점까지 지연처리 한다는 점이다.
  - 어노테이션 변경만으로 원본 객체를 프록시 객체로 대체할 수 있다. 이것이 바로 다형성과 DI 컨테이너가 가진 큰 장점이다.